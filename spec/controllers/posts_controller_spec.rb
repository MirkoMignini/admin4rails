      ...
      context '#index' do
        context 'Global controller' do
          it 'overrides' do
            Admin4rails.dsl.controller do
              index { override ->(_sender) { @test = 'Global Index override' } }
            end
            get(:index)
          ...
          it 'handles before and after events' do
            Admin4rails.dsl.controller do
              index do
                before ->(_sender) { @tb = 'Global Index before' }
                after ->(_sender) { @ta = 'Global Index after' }
              end
            end
            get(:index)
        ...
        context 'Resource based' do
          it 'overrides' do
            resource.define do
              controller! do
                index { override ->(_sender) { @test = 'Resource Index override' } }
              end
            end
            get(:index)
            expect(assigns(:grid)).to be_nil
          ...
          it 'handles before and after events' do
            resource.define do
              controller do
                send(:index) do
                  before ->(_sender) { @tb = 'Resource Index before' }
                  after ->(_sender) { @ta = 'Resource Index after' }
                end
              end
            end
            get(:index)
